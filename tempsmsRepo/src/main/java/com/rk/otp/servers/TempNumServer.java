...
@Service
@PropertySource(value = "classpath:servers/server1.properties", ignoreResourceNotFound = true)
public class TempNumServer implements ServerApi {

	private static final Logger LOGGER = LoggerFactory.getLogger(TempNumServer.class);

	@Value("${tempnum.url.getBalance}")
	private String getBalanceUrl;

	@Value("${tempnum.url.service.price}")
	private String getPriceUrl;

	@Value("${tempnum.url.getnumber}")
	private String getNumberUrl;

	@Value("${tempnum.url.number.getstatus}")
	private String getStatusUrl;

	@Value("${tempnum.url.number.getfullsms}")
	private String getFullSmsUrl;

	@Value("${tempnum.url.number.cancel}")
	private String cancelUrl;

	@Value("${tempnum.url.getPriceAll}")
	private String getPriceAllUrl;
	
	private final RestTemplate template;
	
	public TempNumServer(RestTemplate template) {
